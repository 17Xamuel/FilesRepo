...
if (mysqli_num_rows($exe2) > 0) {
    if ($type == 'super admin') {
        ...
        $mess = 'Super admin already exist';
        header('location: http://localhost/academic/superadmin/add_super_admin.php?mess=' . $mess . '');
    } elseif ($type == 'admin') {
        // redirect to admin with messege
        $mess = 'Admin already exist';
        header('location: http://localhost/academic/superadmin/add_admin.php?mess=' . $mess . '');
    } else {
        // redirect to user with messege
        $mess = 'User already exist';
        header('location: http://localhost/academic/user/add_user.php?mess=' . $mess . '');
    }
} else {
    //lets validat before insert the record in the database

    if (strlen($fisrt_name) < 3 || strlen($last_name) < 3 || strlen($reg_no) < 7 ||strlen($stud_no) < 7 || strlen($password) < 5 || strpos($email, '@') < 6 || strlen($phone) < 10) {
        if ($type == 'super admin') {
            ...
            $mess = 'Invalid input, recheck and try again';
            header('location: http://localhost/academic/admin/add_user.php?mess=' . $mess . '');
        }
    } else {
        ...
        if ($exe3) {
            if ($type == 'super admin') {
                ...
                $mess = 'Regestered successfully';
                header('location: http://localhost/academic/admin/add_users.php?mess=' . $mess . '');
            }
        } else {
